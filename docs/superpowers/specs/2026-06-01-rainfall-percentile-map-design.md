...
| Boundaries overlay | **SA2 only** for this release — reproduces the reference map's outlines + town labels directly from the 26 WA SA2 features. **No `sd` option is exposed** (a known-blocked path is not shipped). SD overlay is a tracked follow-up (see Open Items) that will later add `--regions sa2\|sd`. No numerical regional aggregation in scope. |
...
| Output | `reports/figures/wa_{month}_{year}_percentiles.png` — raster surface, SA2 overlay, title, binned legend, source note, and **baseline period in the caption**. |
...
**Scope: SA2 only for this release.** No `level`/`sd` parameter is built — the SD
overlay is a separate follow-up (see Open Items) and **no `sd` code path is carried
in this release**.

- `load_wheatbelt_regions() -> GeoDataFrame`:
  ...
  - Keep the SA2 name column (`SA2_NAME16`) for labels.
  - Returns geometries in EPSG:4326 to match the SILO grid.
...
- `render_percentile_map(pct_grid, regions, *, month, year, baseline_start, baseline_end, out_path)`:
...
python scripts/plot_rainfall_percentiles.py \
    --month 7 --year 2024 \
    [--baseline-start 1911] [--baseline-end 2025] [--out PATH]
```

**No `--regions` flag in this release** — the overlay is SA2. The flag is added
later when SD lands (`--regions sa2|sd`), so its absence now is forward-compatible.
...
```
CLI (--month --year --baseline-*)
  → boundaries.load_wheatbelt_regions()        → regions + clip_mask
...
| Boundary layer | `load_wheatbelt_regions()` returns **exactly 26** WA polygons with the `SA2_NAME16` label column; CRS is EPSG:4326. `clip_mask` is a single valid (multi)polygon. (SD dissolve tests are part of the SD follow-up, not this release.) |
...
3. **feat: rainfall percentile map (SA2)** — engine + `latest_complete_year`
   helper + SA2 boundaries + renderer + CLI + tests. **SA2-only**: no `sd` code
   path, no `--regions` flag. Fully reproduces the reference map.
...
## Follow-up: SD overlay (separate work, after the SA2 release)

The first release is **SA2-only and ships no `sd` code path**. The SD overlay is
tracked as its own piece of work, in this order:

1. **Add a repo-managed boundary asset or lookup.** Either a committed
   `SA2_2016 → SD` lookup under `data/meta/`, or a committed SD boundary asset
   loaded directly. (The existing `2021-SA2→2011-SD` concordance is unusable here:
   key mismatch with the 2016-SA2 geojson, and it lives at an external absolute
   path outside the repo.)
2. **Define its provenance and license** — source, vintage, ABS license terms —
   documented alongside the asset.
3. **Implement and test the dissolve** — 26 WA SA2 → SD, asserting the expected SD
   count and valid dissolved geometry.
4. **Extend the CLI** with `--regions sa2|sd` (default `sa2`), reusing the renderer
   and clip-mask unchanged.

Until step 1 is resolved, no `sd` option is exposed — a known-blocked path is not
carried in the shipped CLI.
